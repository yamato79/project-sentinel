<template>
    <SwitchGroup as="div" :class="[($slots.default ? 'bg-gray-50 px-4 py-2.5' : ''), (props.disabled ? 'opacity-50' : ''), 'flex items-center justify-between  rounded']">
        ...
        <Switch v-model="modelValue" :class="[modelValue ? 'bg-primary-600' : 'bg-gray-200', 'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none']"  :disabled="props.disabled">
