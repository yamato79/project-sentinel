...
FROM
    (
        SELECT
            ...
            COUNT(*) FILTER (WHERE (monitor_queue.raw_data ->> 'response_code') IS NOT NULL) AS total_responses,
            COUNT(*) FILTER (
                WHERE
                    (monitor_queue.raw_data ->> 'response_code')::integer >= 200 AND
                    (monitor_queue.raw_data ->> 'response_code')::integer <= 299
            ) AS successful_responses
        FROM
            monitor_queue
        WHERE
            (monitor_queue.raw_data ->> 'response_code') IS NOT NULL
        GROUP BY
            monitor_queue.website_id,
